...
class HaroldIsaacLabEnv(DirectRLEnv):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        optimal_air_time = 1.0 #0.15  # Appropriate for Harold's 40cm scale (was 0.3s - too long)
        air_time_error = torch.abs(last_air_time - optimal_air_time)
        air_time_reward = torch.sum(torch.exp(-air_time_error * 3.0) * first_contact, dim=1) * ( #3.0 #10.0
