...
"""Environment configuration for sim_flat_v1 — exact Spot flat locomotion reproduction."""
...
# ---------------------------------------------------------------------------
# Terrain (Spot reference: cobblestone road)
# ---------------------------------------------------------------------------
...
class RewardsCfg:
    ...
    joint_acceleration_weight: float = -1.0e-4  # hip joints only
    ...
    joint_velocity_weight: float = -1.0e-2  # hip joints only
...
# ---------------------------------------------------------------------------
@configclass
class SimFlatV1EnvCfg(DirectRLEnvCfg):
    """Spot flat locomotion — direct env reproduction of Isaac Lab reference."""
    ...
    action_scale = 0.2      # Spot reference: JointPositionAction scale
    ...
    # --- Viewer (close-up camera following the robot) ---
    viewer = ViewerCfg(
        eye=(1.5, 1.0, 0.6),      # Slightly further for Spot (larger robot)
        lookat=(0.0, 0.0, 0.25),
    ...
    # --- Robot ---
    robot = SPOT_CFG.replace(prim_path="/World/envs/env_.*/Robot")
