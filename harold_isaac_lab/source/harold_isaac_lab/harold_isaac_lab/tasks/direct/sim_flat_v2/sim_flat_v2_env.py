...
"""DirectRLEnv for sim_flat_v2 — Harold robot with Spot reward structure.

Identical to sim_flat_v1 except body/joint name patterns are adapted for Harold:
  - Feet: .*calf (not .*_foot)
  - Sync pairs: fl_calf/br_calf + fr_calf/bl_calf (not fl_foot/hr_foot + fr_foot/hl_foot)
  - Hip joints: .*_shoulder_joint + .*_thigh_joint (not .*_h[xy])
  - Undesired contacts: .*body + .*thigh + .*shoulder (not body + .*leg)
"""
...
from .sim_flat_v2_env_cfg import SimFlatV2EnvCfg
from . import train_env


class SimFlatV2Env(DirectRLEnv):
    """Harold robot with Spot reward structure (direct RL).

    Same as SimFlatV1Env but with Harold body/joint naming conventions.
    """

    cfg: SimFlatV2EnvCfg

    def __init__(self, cfg: SimFlatV2EnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # --- Body/joint index lookups (Harold naming) ---
        # Feet: Harold's ground-contact bodies are the calf links
        self._foot_body_ids = self._contact_sensor.find_bodies(".*calf")[0]
        print(f"Foot body IDs: {self._foot_body_ids} "
              f"({[self._contact_sensor.body_names[i] for i in self._foot_body_ids]})")

        # Synced foot pairs for trot gait: (FL, BR) and (FR, BL)
        # Harold uses bl/br prefix; Spot uses hl/hr prefix
        self._sync_pair_0 = (
            self._contact_sensor.find_bodies("fl_calf")[0][0],
            self._contact_sensor.find_bodies("br_calf")[0][0],
        )
        self._sync_pair_1 = (
            self._contact_sensor.find_bodies("fr_calf")[0][0],
            self._contact_sensor.find_bodies("bl_calf")[0][0],
        )

        # Undesired contact bodies: body + thigh + shoulder (for termination)
        ...
        # Hip joint indices: Harold's shoulder + thigh joints (equivalent to Spot's h[xy])
        self._hip_joint_ids = self._robot.find_joints(
            [".*_shoulder_joint", ".*_thigh_joint"]
        )[0]
        ...
        self._foot_body_ids_robot = self._robot.find_bodies(".*calf")[0]
        ...
        print(f"\n{'='*60}")
        print(f"sim_flat_v2: Harold robot with Spot rewards (direct env)")
    ...
    def _apply_startup_randomization(self) -> None:
        """Randomize physics material and base mass.
        ...
        # Clamp to min 0.01 kg to avoid PhysX errors (Harold is much lighter than Spot)
        ...
        if len(body_idx) > 0:
            ...
            masses[:, body_id] = torch.clamp(masses[:, body_id] + mass_delta, min=0.01)
